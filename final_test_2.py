...
for count in range(N):
    if bingo_board[(count - 1) * (N - 1)] != "*": # count + (N * 2 - count)과 같은 소리이다.
        break
else:
    bingo_count += 1
print(bingo_count)
...
while bingo_count < 2:
    ...
    for row in range(N):
        for col in range(N):
            if bingo_board[col + (row * N)] != "*": # 가로줄 넘어가야해서 row에 N을 곱해서 밑에줄 인덱스로 변경함
                break
        else:
            bingo_count += 1
    print(bingo_count)
    ...
    for col in range(N):
        for row in range(N):
            if bingo_board[col + (row * N)] != "*": # 세로줄로 넘어가기 위해 col에 row랑N을 곱한 값을 더해줌
                break
        else:
            bingo_count += 1
    print(bingo_count)
